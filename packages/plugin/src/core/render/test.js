(function () {
    ...
    const px2rem = px => {
      ...
      const htmlElementFontSize = window.getComputedStyle(document.documentElement).fontSize
    ...
    // // 用于替换那些相同节点的值
    const resetSameNodes = ele => {
      if (ele && ele.childNodes && ele.childNodes.length > 1) {
        // 第一个节点做保存
  
        let firstCloneNode
        // 循环
        ele.childNodes.forEach(childNode => {
          if (hasAttr(childNode, 'data-skeleton-map')) {
            if (firstCloneNode && firstCloneNode.tagName === childNode.tagName) {
              // 存在的话 直接进行替换当前子节点为第一个子节点
              ele.replaceChild(firstCloneNode.cloneNode(true), childNode)
            } else {
              // 不存在先赋值
              firstCloneNode = childNode
            }
          }
        })
      }
    ...
    const addAnimationForElement = ele => {
      if (ele) {
        const options = window.AwesomeSkeleton.options || {}
        ele.style.animation = options.animation ? 'skeleton-opacity-animation 1.5s ease infinite' : void 0
      }
    }
    ...
    function buttonHandler (node) {
      ...
      let {backgroundColor: bgColor, width, height} = window.getComputedStyle(node)
    ...
    function styleHandler () {
      ...
      .${PSEUDO_CLASS}::after {
        ...
        animation: skeleton-opacity-animation 1.5s ease infinite
      }
    ...
    function beforeHandler (node, options) {
      ...
      const isHidden = width < options.minGrayBlockWidth
      if (isHidden) {
        setOpacity(node)
      }
      ...
      if (ComputedStyle.backgroundImage !== 'none') {
        ...
        // 增加动画效果
        !isHidden && addAnimationForElement(node)
      }
    ...
    window.AwesomeSkeleton = {
      ...
      async startGenSkeleton () {
        const { skeletonRoot } = this.options
        let handleNodes = document.querySelectorAll(skeletonRoot) || [document.body]
        if (Array.isArray(handleNodes) && handleNodes.length > 1) {
          console.error(`骨架屏root节点只能存在一个，当前${skeletonRoot}存在多个将自动切换为body节点`)
          handleNodes = [document.body]
        }
        this.init()
        try {
          this.handleNode(handleNodes[0])
        } catch (e) {
